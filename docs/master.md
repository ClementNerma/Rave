...
To get a map with sorting (by default in natural order), we have to use the `SortedMap<K, V>` class. It inherits from `Map<K, V>` and so it has all its methods, but this one is sorted - the only downside being a performance cost, because values must be compared at insertion time.
